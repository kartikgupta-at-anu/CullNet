...
class Dataset_class(Dataset):
    def __init__(self, imdb_name, class_name, datadir, transform, dst_size, occlusion=False):
        ...
        if not occlusion:
            ...
            if cfg.args.num_detection_points==9:
                occ_datadirname = 'singleobject_occtest'
                self._data_path = os.path.join(datadir, occ_datadirname, class_name)
                self._data_dir = self._data_path
                self._classes = cfg.label_names
    ...
    def __getitem__(self, index):
        if self._name == 'linemod_trainerror' or self._name == 'linemod_test' or self._name == 'linemod_val' \
                or self._name == 'occlusion_test':
            image, origin_im, gt_boxes, origin_gt_boxes, gt_classes, gt_RT = self.transform(
                [self._image_names[index], self._annotations[index]], self._dst_size)
            sample = {'image': image, 'origin_im': origin_im, 'gt_boxes': gt_boxes,
                      'origin_gt_boxes': origin_gt_boxes,
                      'gt_classes': gt_classes,  'gt_RT': gt_RT}
        else:
            image, gt_boxes, gt_classes, dontcare, origin_im, gt_RT = self.transform(
                [self._image_names[index], self._annotations[index]])
            sample = {'image': image, 'gt_boxes': gt_boxes, 'gt_classes': gt_classes, 'dontcare': dontcare,
                      'origin_im': origin_im, 'gt_RT': gt_RT}
        return sample
