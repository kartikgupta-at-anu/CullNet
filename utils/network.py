...
def tensor_to_variable(x, is_cuda=True, dtype=torch.FloatTensor, volatile=False):
    if volatile:
        v = Variable(x.type(dtype), requires_grad = False)
    else:
        v = Variable(x.type(dtype), requires_grad = True)
